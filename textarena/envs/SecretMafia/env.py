...
class SecretMafiaEnv(ta.Env):
    ...
    def _transition_current_pid(self, his):
        ...
        if not self.next_player_ids:
            ...
            if new_phase=="Day-Discussion" or new_phase=="Night-Mafia":
                ...
                winners = self._check_winning_conditions()
                if winners:
                    self.his += f"{winners}  wins!<game_over>\n"
                    self.game_idx += 1
                    self.step_idx = 1
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if self.step_idx == 1:
            his = (f"<game_begin>Game {self.game_idx}:"
                    f"player {current_pid} took action: {action}.\n")
        else:
            his = f"player {current_pid} took action: {action}.\n"
        # check game phase 
